...
define("cdg", [], function() {
  ...
  function CDGPlayer(containerId, initOptions) {
    ...
    function handleCDGHttpRequest() {
      ...
      if (cdgHttpRequest.readyState == 4) {
        if (cdgHttpRequest.status != 200) {
          console.error('CDG file failed to load');
          return;
        }
    ...
    function init(containerId, initOptions) {
      ...
      buttonPanelEl.id=containerId + "-buttonPanel"
      borderEl.appendChild(canvasEl);
      containerEl.appendChild(borderEl);
      containerEl.appendChild(buttonPanelEl);
      buttonPanelEl.appendChild(audioPlayer);

      if(initOptions.nextTrack)
      {
        buttonPanelEl.appendChild(initOptions.nextTrack.el);
        audioPlayer.style.width = (canvasEl.offsetWidth - initOptions.nextTrack.el.offsetWidth) + "px";
      }
      else
      {
